...
class Course(models.Model):
    ...
    degree=models.CharField('难度',choices=DEGREE_CHOICES,max_length= 300)
    learn_times=models.IntegerField("学习时长（分钟数）",default=0)
    students=models.IntegerField('学习人数',default=0)
    fav_nums=models.IntegerField('收藏人数',default=0)
    image=models.ImageField("封面图",upload_to="course/%Y/%m",max_length=100)
    click_nums=models.IntegerField('点击书',default=0)
...
class Lesson(models.Model):
    ...
    def __str__(self):
        return '<{0}>课程的章节>>{1}'.format(self.course,self.name)  #一对多的关系中，通过外键在多的一方
